...
async function genCmd(fshare_folder, remote_drive, remote_path, page=1, is_root_folder=true) {
	...
	try {
		...
		const promises = body.items.map(async item => {
			if (item.type === 1) {
				let cmd = `curl -s https://raw.githubusercontent.com/tungtran96/fshare/master/fshare2gdrive.js | tail -n+2 | node - "https://fshare.vn/file/${item.linkcode}" "${remote_drive}" "${remote_path.replace(/\/$/,'')}/${(is_root_folder ? body.current.name + '/' : '')}" | bash -s`
				console.log(cmd)
			}	else {
